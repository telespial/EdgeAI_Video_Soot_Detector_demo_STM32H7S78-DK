...
/**
  * @brief  GUI threads initialization function
  * @param  MemoryPtr  Memory pointer
  * @retval Functional state
  */
uint32_t GUI_InitOS(void *MemoryPtr)
{
  static uint8_t _init = 0U;
  GUIOS_INIT();
  ...
  if (_init == 0U)
  {
    ...
    GUI_Start();
#if defined(USE_STM32_UTILITY_OS)
    UTIL_SEQ_RegTask(TASK_GUI, 0U, GUI_Execute);
    UTIL_SEQ_SetTask(TASK_GUI, 0U);
#endif /* USE_STM32_UTILITY_OS */
#endif /* _RTOS || USBPD_THREADX */
  }
...
/**
  * @brief  GUI reset by erasing data in flash
  * @retval None
  */
void GUI_Reset(void)
{
  /* Erase all saved data in flash */
  BSP_GUI_EraseDataInFlash();
}

/**
  * @brief  GUI start reception
  * @retval None
  */
void GUI_Start(void)
{
...
#endif /* _RTOS || USBPD_THREADX */
        for (uint8_t _instance = 0U; _instance < (uint8_t)USBPD_PORT_COUNT; _instance++)
        {
          /* -------------------------------------------------  */
          ...
          else
          {
            ...
            GUI_TimerMeasReport[_instance] = 0U;
          }
        }
#if defined(_RTOS) || defined(USBPD_THREADX)
        break;
...
#if defined(_RTOS) || defined(USBPD_THREADX)
/**
  * @brief  Calculate minimum timers to wake-up GUI task
  * @retval Timing
  */
static uint32_t CheckGUITimers(void)
{
...
/**
  * @brief  Increment GUI timer counter
  * @retval None
  */
...
/**
  * @brief      Execution of the received message
  * @param      PortNum     Port number
  * @param      instruction Pointer on the instruction
  * @param      pEncodedMsg Pointer on the encoded message
...
/**
  * @brief  Send a DPM_CONFIG_GET_CNF message to the GUI with the current configuration of the specified port.
  * @param  PortNum     Current port number.
  * @param  instruction TLV instruction received.
  * @param  pEncodedMsg Pointer to the encoded message.
  * @retval None
  */
static void Send_DpmConfigGetCnf(uint8_t PortNum, uint8_t *instruction, uint8_t *pEncodedMsg)
{
...
/**
  * @brief   Send DPM_CONFIG_REJ message to GUI with reason of rejection
  * @param   PortNum      Port number
  * @param   pEncodedMsg  Pointer on the encoded message
  * @param   RejectReason Reason of rejection
  * @retval  None
  */
...
/**
  * @brief   Send DPM_MESSAGE_REJ message to GUI with reason of rejection
  * @param   PortNum      Port number
  * @param   pEncodedMsg  Pointer on the encoded message
  * @param   RejectReason Reason of rejection
  * @retval  None
...
/**
  * @brief   Send DPM_REGISTER_READ_CNF message to GUI
  * @param   PortNum      Port number
  * @param   pEncodedMsg  Pointer on the encoded message
