...
/** @defgroup USBPD_CORE_GUI_TYPE_NOTIFICATION USBPD CORE TRACE  TYPE NOTIFICATION
  * @{
  */
#define  GUI_NOTIF_NUMBEROFRCVSNKPDO                    (1UL << 0U)
#define  GUI_NOTIF_RDOPOSITION                          (1UL << 1U)
#define  GUI_NOTIF_LISTOFRCVSRCPDO                      (1UL << 2U)
#define  GUI_NOTIF_NUMBEROFRCVSRCPDO                    (1UL << 3U)
#define  GUI_NOTIF_LISTOFRCVSNKPDO                      (1UL << 4U)
#define  GUI_NOTIF_ISCONNECTED                          (1UL << 5U)
#define  GUI_NOTIF_DATAROLE                             (1UL << 6U)
#define  GUI_NOTIF_POWERROLE                            (1UL << 7U)
#define  GUI_NOTIF_CCDEFAULTCURRENTADVERTISED           (1UL << 8U)
#define  GUI_NOTIF_VCONNON                              (1UL << 9U)
#define  GUI_NOTIF_VCONNSWAPED                          (1UL << 10U)
#define  GUI_NOTIF_MEASUREREPORTING                     (1UL << 11U)
#define  GUI_NOTIF_CC                                   (1UL << 12U)
#define  GUI_NOTIF_PE_EVENT                             (1UL << 13U)
#define  GUI_NOTIF_TIMESTAMP                            (1UL << 14U)
#define  GUI_NOTIF_POWER_EVENT                          (1UL << 15U)
...
typedef struct
{
  uint16_t VBUS_Level               :16U;  /*!< VBUS Level */
  uint16_t IBUS_Level               :16U;  /*!< IBUS Level */
  ...
  union
  {
    ...
    struct
    {
      uint8_t MeasReportValue       :7U;   /*!< Enable Measure reporting every tr x 40 ms  */
      uint8_t MeasReportActivation  :1U;   /*!< Enable or Disable Measure reporting        */
    } d;
  } u;
  uint8_t Reserved                  :8U;   /*!< Reserved bits */
...
typedef struct
{
...
#else
  uint32_t            Reserved_ReqPower[6U];                    /*!< Reserved bits to match with Resquested power information            */
#endif /* _SNK || _DRP */
#if _MANU_INFO
...
#else
  uint32_t            ReservedManu[7U];                         /*!< Reserved bits to match with Manufacturer information            */
#endif /* _MANU_INFO */
#if _SRC_CAPA_EXT && (defined(_SRC)||defined(_DRP))
  USBPD_SCEDB_TypeDef DPM_SRCExtendedCapa;                      /*!< SRC Extended Capability                               */
#else
  uint32_t            ReservedSrcCapa[6U];                      /*!< Reserved bits to match with SrcCapa information            */
#endif /* _SRC_CAPA_EXT && (_SRC || _DRP) */
#if defined(_SNK)||defined(_DRP)
  USBPD_SKEDB_TypeDef DPM_SNKExtendedCapa;                      /*!< SNK Extended Capability                                */
  uint8_t             ReservedSnkCapa[3U];                      /*!< Reserved bits to match with SnkCapaExt information     */
#else
  uint32_t            ReservedSnkCapa[6U];                      /*!< Reserved bits to match with SnkCapaExt information     */
#endif /* _SNK || _DRP */
  ...
  uint32_t CAD_tDRP                                       :7U;  /*!< The period that DRP shall complete a Source to Sink and back advertisement */
  uint32_t CAD_dcSRC_DRP                                  :7U;  /*!< The percent of time that a DRP shall advertise Source during tDRP (in %) */
...
typedef struct
{
  uint32_t VDM_XID_SOP                      :32U; /*!< A decimal number assigned by USB-IF before certification */
  uint32_t VDM_USB_VID_SOP                  :16U; /*!< A unique 16-bit number, assigned to the Vendor by USB-IF. */
  uint32_t VDM_PID_SOP                      :16U; /*!< A unique number assigned by the Vendor ID holder identifying the product. */
  uint32_t VDM_bcdDevice_SOP                :16U; /*!< A unique number assigned by the Vendor ID holder containing identity information relevant to the release version of the product. */
  USBPD_ModalOp_TypeDef VDM_ModalOperation  : 1U; /*!< Product support Modes based on @ref USBPD_ModalOp_TypeDef */
  USBPD_USBCapa_TypeDef VDM_USBHostSupport  : 1U; /*!< Indicates whether the UUT is capable of enumerating USB Host */
  USBPD_USBCapa_TypeDef VDM_USBDeviceSupport: 1U; /*!< Indicates whether the UUT is capable of enumerating USB Devices */
  USBPD_ProductType_TypeDef VDM_ProductTypeUFPorCP : 3U; /*!< Product type UFP or CablePlug of the UUT based on @ref USBPD_ProductType_TypeDef */
  USBPD_ProductType_TypeDef VDM_ProductTypeDFP : 3U; /*!< Product type DFP of the UUT based on @ref USBPD_ProductType_TypeDef */
  uint32_t Reserved3                        : 7U; /*!< Reserved bits */
