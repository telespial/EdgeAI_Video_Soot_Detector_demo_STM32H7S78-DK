...
/* USER CODE BEGIN PFP */
static void MPU_Config(void);
static void RibbonCamera_Probe(void);
static void ForceUsb1VbusOn(void);

/* USER CODE END PFP */

/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
volatile uint32_t g_ribbon_cam_probe_ok = 0U;
volatile uint32_t g_ribbon_cam_id = 0U;
volatile uint32_t g_ribbon_cam_last_i2c_status = 0xFFFFFFFFUL;
volatile uint32_t g_ribbon_cam_probe_count = 0U;
volatile int32_t g_usb1_vbus_force_init_rc = 0;
volatile int32_t g_usb1_vbus_force_vbus_init_rc = 0;
volatile int32_t g_usb1_vbus_force_role_rc = 0;
volatile int32_t g_usb1_vbus_force_on_rc = 0;
...
/* MPU Configuration--------------------------------------------------------*/
  ...
  /* Initialise Demo queues */
  demoQueue_USBPD_Event = osMessageQueueNew(30, sizeof(int),NULL);
  /* USER CODE END 2 */

  /* Init scheduler */
  osKernelInitialize();
  /* USBPD policy stack is disabled for forced USB1 host-power camera mode. */
  /* MX_USBPD_Init(); */
 ...
 /* creation of TouchGFXTask */
  ...
  /* Force USB1 host-only operation for webcam bring-up. */
  /* USB_DEVICE_RTOS_Init(); */
  USB_HOST_RTOS_Init();
  /* Start scheduler */
  osKernelStart();
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pins : PM12 PM11 PM13 */
  ...
  /*Configure GPIO pins : PM6 PM9 PM5 */
  ...
  /* PM9 drives TCPP0203 PWRENn on this DK; keep as GPIO output, not OTG AF. */
...
static void RibbonCamera_Probe(void)
{
  uint8_t idh = 0U;
  uint8_t idl = 0U;
  HAL_StatusTypeDef st_h;
  HAL_StatusTypeDef st_l;

  g_ribbon_cam_probe_count++;
  g_ribbon_cam_probe_ok = 0U;
  g_ribbon_cam_id = 0U;
  g_ribbon_cam_last_i2c_status = 0xFFFFFFFFUL;

  st_h = HAL_I2C_Mem_Read(&hi2c1, CAMERA_OV5640_ADDRESS, 0x300A, I2C_MEMADD_SIZE_16BIT, &idh, 1U, 100U);
  st_l = HAL_I2C_Mem_Read(&hi2c1, CAMERA_OV5640_ADDRESS, 0x300B, I2C_MEMADD_SIZE_16BIT, &idl, 1U, 100U);
  g_ribbon_cam_last_i2c_status = ((uint32_t)st_h << 16) | (uint32_t)st_l;

  if ((st_h == HAL_OK) && (st_l == HAL_OK))
  {
    g_ribbon_cam_id = ((uint32_t)idh << 8) | (uint32_t)idl;
    if (g_ribbon_cam_id == 0x5640U)
    {
      g_ribbon_cam_probe_ok = 1U;
    }
  }
}
...
static void ForceUsb1VbusOn(void)
{
  g_usb1_vbus_force_init_rc = BSP_USBPD_PWR_Init(USBPD_PWR_TYPE_C_PORT_1);
  g_usb1_vbus_force_vbus_init_rc = BSP_USBPD_PWR_VBUSInit(USBPD_PWR_TYPE_C_PORT_1);
  g_usb1_vbus_force_role_rc = BSP_USBPD_PWR_SetRole(USBPD_PWR_TYPE_C_PORT_1, POWER_ROLE_SOURCE);
  g_usb1_vbus_force_on_rc = BSP_USBPD_PWR_VBUSOn(USBPD_PWR_TYPE_C_PORT_1);
}
