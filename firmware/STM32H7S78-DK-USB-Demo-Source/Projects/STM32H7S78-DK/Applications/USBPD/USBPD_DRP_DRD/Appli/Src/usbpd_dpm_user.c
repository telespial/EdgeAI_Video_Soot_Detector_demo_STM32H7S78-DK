...
QueueHandle_t demoQueue_USBPD_Event;
  ...
  case USBPD_NOTIFY_DATAROLESWAP_DFP :
    {
      if( USBPD_TRUE == DPM_USER_Settings[PortNum].PE_DR_Swap_To_DFP)
      {
        ...
        Demo_Message = DATA_CHANGE_TO_DFP;
        osMessageQueuePut(demoQueue_USBPD_Event, &Demo_Message, 0,0);
      }
      break;
    }
  case USBPD_NOTIFY_DATAROLESWAP_UFP :
    {
      if( USBPD_TRUE == DPM_USER_Settings[PortNum].PE_DR_Swap_To_UFP)
      {
        ...
        Demo_Message = DATA_CHANGE_TO_UFP;
        osMessageQueuePut(demoQueue_USBPD_Event, &Demo_Message, 0,0);
      }
      break;
    }
                                     ...
                                     USBPD_SNKRDO_TypeDef* Rdo, USBPD_CORE_PDO_Type_TypeDef *PtrPowerObject)
{
  ...
  rdo.GenericRDO.NoUSBSuspend                 = pdo.SRCFixedPDO.USBSuspendSupported;
